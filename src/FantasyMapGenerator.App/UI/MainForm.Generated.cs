/* Generated by MyraPad at 4/9/2023 2:27:25 AM */
using Myra;
using Myra.Graphics2D;
using Myra.Graphics2D.TextureAtlases;
using Myra.Graphics2D.UI;
using Myra.Graphics2D.Brushes;
using Myra.Graphics2D.UI.Properties;
using FontStashSharp.RichText;
using Microsoft.Xna.Framework;
using Microsoft.Xna.Framework.Graphics;

namespace FantasyMapGenerator.App.UI
{
	partial class MainForm: VerticalStackPanel
	{
		private void BuildUI()
		{
			_buttonRun = new TextButton();
			_buttonRun.Text = "Run";
			_buttonRun.Width = 100;
			_buttonRun.Id = "_buttonRun";

			_buttonStep = new TextButton();
			_buttonStep.Text = "Step";
			_buttonStep.Padding = new Thickness(8, 0);
			_buttonStep.Id = "_buttonStep";

			var horizontalStackPanel1 = new HorizontalStackPanel();
			horizontalStackPanel1.Spacing = 9;
			horizontalStackPanel1.Widgets.Add(_buttonRun);
			horizontalStackPanel1.Widgets.Add(_buttonStep);

			var horizontalSeparator1 = new HorizontalSeparator();

			_panelProperties = new Panel();
			_panelProperties.ClipToBounds = true;
			_panelProperties.Id = "_panelProperties";

			_panelMap = new Panel();
			_panelMap.ClipToBounds = true;
			_panelMap.Id = "_panelMap";

			_panelLog = new Panel();
			_panelLog.Id = "_panelLog";

			var panel1 = new Panel();
			panel1.Widgets.Add(_panelMap);
			panel1.Widgets.Add(_panelLog);

			_splitPane = new HorizontalSplitPane();
			_splitPane.Id = "_splitPane";
			_splitPane.Widgets.Add(_panelProperties);
			_splitPane.Widgets.Add(panel1);

			
			Proportions.Add(new Proportion
			{
				Type = Myra.Graphics2D.UI.ProportionType.Auto,
			});
			Proportions.Add(new Proportion
			{
				Type = Myra.Graphics2D.UI.ProportionType.Auto,
			});
			Proportions.Add(new Proportion
			{
				Type = Myra.Graphics2D.UI.ProportionType.Fill,
			});
			Widgets.Add(horizontalStackPanel1);
			Widgets.Add(horizontalSeparator1);
			Widgets.Add(_splitPane);
		}

		
		public TextButton _buttonRun;
		public TextButton _buttonStep;
		public Panel _panelProperties;
		public Panel _panelMap;
		public Panel _panelLog;
		public HorizontalSplitPane _splitPane;
	}
}
